...
void MPItest(int rank, int nproc, int size, double *buf, double *buf2){
	...
	double *recv0buf=(double*)malloc(sizeof(double)*size);
	double *recv1buf=(double*)malloc(sizeof(double)*size);

	if(rank == 0) {
		MPI_Send(buf, size, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD);
		MPI_Recv(recv0buf, size, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	}
	else if(rank == 1) {
		MPI_Recv(recv1buf, size, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Send(buf2, size, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
	}
}
