...
*/
	...
	for(k = 0; k < nproc_row; k++) {
		if(row_rank == (col_rank + k) % nproc_row) {
			MPI_Bcast(&(mA[0][0]), np * np , MPI_DOUBLE, (col_rank + k) % nproc_row, row_comm); 
			for(i = 0; i < np; i++) {
				for(j = 0; j < np; j++) {
					for(l = 0; l < np; l++) {
						C[i][j] += mA[i][l] * mB[l][j];
			...
			MPI_Bcast(&(temp[0][0]), np * np, MPI_DOUBLE, (col_rank + k) % nproc_row, row_comm); 
			for(i =0; i < np; i++) {
				for(j = 0; j < np; j++) {
					for(l = 0; l < np; l++) {
						C[i][j] += temp[i][l] * mB[l][j];
					}
		...
		if(col_rank - 1 < 0) {
			MPI_Sendrecv_replace(&(mB[0][0]), np * np, MPI_DOUBLE, nproc_row - 1, 0, (col_rank + 1) % nproc_row, 0, col_comm, &status);
		}
		else { 
			MPI_Sendrecv_replace(&(mB[0][0]), np * np, MPI_DOUBLE, (col_rank - 1) % nproc_row , 0, (col_rank + 1) % nproc_row, 0, col_comm, &status);
		}
